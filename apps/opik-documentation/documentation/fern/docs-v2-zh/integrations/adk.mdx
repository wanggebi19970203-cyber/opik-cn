---
description: 开始将 Opik 集成到基于 Google Agent Development Kit 的生成式 AI 应用中，实现端到端的 LLM 可观测性、单元测试和优化。
headline: Google ADK | Opik 文档
og:description: 使用 Google ADK 构建灵活的 AI 代理，轻松集成 Gemini 模型和 Google AI 工具，支持简单和复杂架构。
og:site_name: Opik 文档
og:title: 使用 Google ADK 开发 AI 代理 - Opik
title: 使用 Opik 实现 Google Agent Development Kit (Python) 的可观测性
---
...
[Agent Development Kit (ADK)](https://google.github.io/adk-docs/) 是一个灵活且模块化的框架，用于开发和部署 AI 代理。ADK 可与流行的 LLM 和开源生成式 AI 工具配合使用，专为与 Google 生态系统和 Gemini 模型紧密集成而设计。ADK 使得使用 Gemini 模型和 Google AI 工具构建简单代理变得容易，同时为更复杂的代理架构和编排提供所需的控制和结构。

本指南将展示如何将 Opik 与 Google ADK 集成，使所有 ADK 调用都被记录为 Opik 中的追踪。我们将介绍三种关键集成模式：
...
[Comet](https://www.comet.com/site?from=llm&utm_source=opik&utm_medium=colab&utm_content=google-adk&utm_campaign=opik) 提供 Opik 平台的托管版本，[只需创建账户](https://www.comet.com/signup?from=llm&utm_source=opik&utm_medium=colab&utm_content=google-adk&utm_campaign=opik) 并获取您的 API Key。
...
Opik 提供与 ADK 的全面集成，自动记录所有代理执行、工具调用和 LLM 交互的追踪，并提供详细的成本跟踪和错误监控。

## 核心特性

- **单行插桩** 使用 `track_adk_agent_recursive` 自动追踪整个代理层级
- **自动成本跟踪** 支持所有主要 LLM 提供商，包括 LiteLLM 模型（OpenAI、Anthropic、Google AI、AWS Bedrock 等）
- **完全兼容** `@opik.track` 装饰器，支持混合追踪方式
- **线程支持** 用于使用 ADK 会话的对话式应用
...
追踪 ADK 代理的推荐方式是使用 [`track_adk_agent_recursive`](https://www.comet.com/docs/opik/python-sdk-reference/integrations/adk/track_adk_agent_recursive.html) 和 [`OpikTracer`](https://www.comet.com/docs/opik/python-sdk-reference/integrations/adk/OpikTracer.html)，通过单个函数调用自动插桩整个代理层级。此方式适用于单代理和复杂的多代理场景：
...
如需对插桩的回调进行细粒度控制，可以手动配置 [`OpikTracer`](https://www.comet.com/docs/opik/python-sdk-reference/integrations/adk/OpikTracer.html) 回调。此方式提供显式控制，但需要更多设置代码：

```python
# 配置 Opik tracer（与之前相同）
opik_tracer = OpikTracer(
    name="basic-weather-agent",
    tags=["basic", "weather", "time", "single-agent"],
    metadata={
...
<Note>
对于大多数用例，我们建议使用 `track_adk_agent_recursive`（如示例 1 所示），因为它需要更少的代码并能自动处理复杂的代理层级。
</Note>

## 示例 3：多代理设置与层级追踪

此示例展示了一个复杂的多代理设置，其中包含针对不同任务的专用代理。使用 `track_adk_agent_recursive`，您可以通过单个函数调用插桩整个层级：
...
通过在顶层代理上调用一次 `track_adk_agent_recursive`，所有子代理及其操作都会自动插桩，无需额外代码

## 成本跟踪

Opik 自动跟踪代理执行期间所有 LLM 调用的令牌使用量和成本，不仅包括 Gemini LLM，还包括通过 `LiteLLM` 访问的模型。
...
Opik 自动生成使用 Mermaid 图表的代理工作流可视化。图表显示：
...
图表会自动计算并存储在每次追踪中，为代理执行流程提供清晰的视觉理解：
...
对于更复杂的代理架构，显示图表可能更有益：

<Frame>
  <img src="/img/tracing/adk/adk_code_assistant_graph_screenshot.png" />
</Frame>

## 示例 4：混合追踪 - 结合 Opik 装饰器与 ADK 回调

此高级示例展示如何将 Opik 的 `@opik.track` 装饰器与 ADK 的回调系统结合使用。当您有执行自身内部操作的复杂多步骤工具，并且希望单独追踪这些操作，同时保持整体代理追踪上下文时，这种方式非常有用。
...
# 使用 track_adk_agent_recursive 对代理进行插桩
# 工具中的 @opik.track 装饰器会自动创建子 span
from opik.integrations.adk import track_adk_agent_recursive
...
您可以在另一个被追踪的函数内部调用代理，也可以在工具函数内部调用被追踪的函数，所有 span 和追踪的父子关系都会被保留！

## 线程支持

Opik 集成自动处理 ADK 会话，并将其映射到 Opik 线程，用于对话式应用：
...
# 所有追踪将自动按 session_id 作为 thread_id 分组
```
...
使用 `Runner.run_async` 时，请确保完整处理所有事件，即使在找到最终响应后（当 `event.is_final_response()` 为 `True` 时）。如果过早退出循环，OpikTracer 将无法记录最终响应，您的追踪将不完整。不要使用过早停止处理事件的代码：
...
有一个上游讨论关于如何最好地解决这个混淆来源：https://github.com/google/adk-python/issues/1695。

<Tip>
  我们的团队已尝试解决这些问题，并使集成尽可能稳健。如果您遇到类似问题，
  我们建议首先将 `opik` 和 `google-adk` 更新到最新版本。我们正在积极改进此集成，
  因此使用最新版本您很可能会获得最佳体验！
</Tip>

## 刷新追踪

`OpikTracer` 对象有一个 `flush` 方法，确保在退出脚本前所有追踪都已记录到 Opik 平台：
...
`OpikTracer` 可以与 [Opik prompt 库](/development/prompt-library/getting-started) 一起使用，
轻松访问现有 prompt 或创建新 prompt，然后将它们与 ADK 代理流程中的追踪或 span 关联。
