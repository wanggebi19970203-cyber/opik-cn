---
description: 了解如何使用自定义算法扩展 Opik 的优化框架，并为项目开发做出贡献。
headline: 扩展优化器 | Opik 文档
og:description: 使用灵活策略为 Opik Agent 构建自定义优化器，并无缝集成到 Opik 的评估生态系统中。
...
Opik Agent Optimizer 被设计为一个灵活的提示词和代理优化框架。虽然它提供了一套强大的内置算法，但您可能有独特的优化策略或特殊需求。本指南展示如何通过扩展所有内置优化器使用的 `BaseOptimizer` 类来构建自己的优化器。
...
要在 Opik 生态系统中设计新的优化算法，您的优化器需要与几个关键组件交互：

1. **提示词（`ChatPrompt`）**：您的优化器接受一个 `ChatPrompt` 对象作为输入。聊天提示词是一个消息列表，每条消息都有角色、内容和可选的附加字段。这包括需要用实际值替换的变量。

2. **评估机制（指标和数据集）**：您的优化器需要一种方式来评分候选提示词。这通过创建一个 `metric`（接受 `dataset_item` 和 `llm_output` 作为参数并返回浮点数的函数）和一个评估 `dataset` 来实现。

3. **优化循环**：这是自定义优化器的核心。它涉及：
   - **候选生成**：创建新提示词变体的逻辑。这可以是基于规则的、LLM 驱动的或基于任何其他启发式方法。
   - **候选评估**：使用 `metric` 和 `dataset` 获取每个候选的分数。
   - **选择/进阶**：决定保留哪些候选、进一步优化哪些候选，或如何根据分数调整生成策略的逻辑。
   - **终止条件**：何时停止优化的标准（例如，轮数、分数阈值、无改进）。

4. **返回结果（`OptimizationResult`）**：完成后，您的优化器返回一个 `OptimizationResult` 对象，标准化结果的报告方式，包括找到的最佳提示词、其分数、优化过程历史和成本/使用指标。
...
创建新提示词变体的自定义逻辑。使用 `get_prompt()` 访问内部提示词（会尊重用户的 `prompt_overrides`）：
...
<Info>
  `BaseOptimizer` 类为所有现有优化器利用的提示词评估提供了强大的机制。您的自定义优化器重用这些内部评估工具，以确保与 Opik 生态系统的一致性。
</Info>

| 组件 | 描述 |
|-----------|-------------|
| `evaluate_prompt()` | 使用指标针对数据集评估提示词。处理线程、采样和结果聚合。 |
| `get_prompt(key, **fmt)` | 获取内部提示词模板，支持可选格式化。尊重 `prompt_overrides`。 |
| `list_prompts()` | 列出此优化器所有可用的提示词键。 |
| `_reset_counters()` | 重置 LLM 调用/成本计数器。在 `optimize_prompt()` 开始时调用。 |
| `_add_to_history()` | 跟踪优化轮次以用于结果报告。 |
| `_should_skip_optimization()` | 检查基线分数是否超过 `perfect_score` 阈值。 |
| `_build_early_result()` | 跳过优化时创建 `OptimizationResult`。 |
| `llm_call_counter` | 跟踪 LLM 调用次数。 |
| `llm_cost_total` | 跟踪总 API 成本（当提供商可用时）。 |
| `llm_token_usage_total` | 跟踪所有调用的 token 使用量。 |

## 使用结构化输出

对于复杂的生成，使用 Pydantic 模型进行结构化 LLM 响应：
...
- **功能请求和想法**：如果您对新优化算法、功能或对现有功能的改进有想法，请通过我们的社区渠道或在我们的 [GitHub 仓库](https://github.com/comet-ml/opik)上提出问题来分享。
- **错误报告**：如果您遇到问题或意外行为，详细的错误报告将不胜感激。
- **用例和反馈**：分享您的用例以及 Opik Agent Optimizer 如何（或未能）满足您的需求，有助于我们优先安排开发工作。
- **代码贡献**：欢迎提交新优化器的拉取请求！请参阅[贡献指南](/v1/contributing/guides/agent-optimizer-sdk)了解详细说明。

## 关键要点

- 扩展 `BaseOptimizer` 以创建自定义优化算法，完全访问 Opik 的基础设施
- 为您的算法内部提示词定义 `DEFAULT_PROMPTS` - 用户可以通过 `prompt_overrides` 自定义它们
- 使用您的优化逻辑实现 `optimize_prompt()`，使用继承的 `evaluate_prompt()` 对候选进行评分
- 返回标准化的 `OptimizationResult` 对象，以实现一致的报告和仪表板集成
- 使用 `_llm_calls.call_model()` 进行 LLM 交互，自动跟踪成本/使用量

我们鼓励您探索现有的[优化器算法](/v1/agent_optimization/algorithms/overview)，了解应对这些挑战的不同方法。

## 相关内容

- [自定义优化器提示词](/v1/agent_optimization/advanced/prompt_customization) - 自定义内部提示词
- [自定义指标](/v1/agent_optimization/advanced/custom_metrics) - 构建评估指标
