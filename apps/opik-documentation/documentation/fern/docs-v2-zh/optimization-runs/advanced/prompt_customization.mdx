...
Opik Optimizer 使用一个 **PromptLibrary** 系统，让您可以自定义每个优化器使用的内部提示词。当您需要以下功能时，这很有用：
...
每个优化器定义自己的 `DEFAULT_PROMPTS` 字典，其中包含特定于该算法的键。PromptLibrary：
...
某些提示词包含占位符，使用 Python 的 `{variable}` 格式在运行时填充。当覆盖带有占位符的提示词时，**保持相同的占位符**：
...
<Info>
  **有效提示词自定义的技巧：**
  ...
  1. **先列出键** – 覆盖前始终使用 `list_prompts()` 查看可用键
  2. **保留占位符** – 如果提示词有 `{variables}`，在覆盖中保留它们
  3. **增量测试** – 每次覆盖一个提示词以隔离效果
  4. **对复杂逻辑使用可调用对象** – 字典更简单，但可调用对象更强大
  5. **不要破坏 JSON** – 某些提示词期望 JSON 输出；保持该结构
</Info>
...
- [自定义指标](/v1/agent_optimization/advanced/custom_metrics) – 构建专门的评估指标
- [扩展优化器](/v1/agent_optimization/advanced/extending_optimizers) – 创建自定义优化器子类
