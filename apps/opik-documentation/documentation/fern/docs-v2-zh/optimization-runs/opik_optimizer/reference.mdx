...
- **多种算法**：支持各种优化策略，包括进化、few-shot、meta-prompt 和 GEPA
- **MCP 支持**：内置 Model Context Protocol 工具调用和优化支持
- **一致的结果**：所有优化器返回标准化的 `OptimizationResult` 对象
- **计数器跟踪**：内置 LLM 和工具调用计数器，用于监控使用情况
- **向后兼容**：通过 kwargs 提取保留所有原始参数
- **弃用警告**：对弃用参数提供清晰的警告和迁移指导

## 核心类

SDK 提供了多个优化器类，它们都继承自 `BaseOptimizer` 并实现相同的标准化接口：

- **ParameterOptimizer**：使用贝叶斯优化优化 LLM 调用参数（temperature、top_p 等）
- **FewShotBayesianOptimizer**：使用 few-shot 学习与贝叶斯优化
- **MetaPromptOptimizer**：采用 meta-prompting 技术进行优化
- **EvolutionaryOptimizer**：使用遗传算法进行提示词进化
- **GepaOptimizer**：利用 GEPA（Genetic-Pareto）优化方法
- **HRPO（分层反思提示词优化器）**：使用分层根因分析进行有针对性的提示词优化
