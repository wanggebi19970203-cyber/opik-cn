...
public record Prompt(
        ...
        @JsonView({Prompt.View.Public.class, Prompt.View.Write.class,
                Prompt.View.Detail.class}) @Schema(description = "项目ID。同时提供 project_name 时，以 project_id 为准。") @Nullable UUID projectId,
        @JsonView({
                Prompt.View.Write.class}) @Pattern(regexp = NULL_OR_NOT_BLANK, message = "must not be blank") @Schema(description = "项目范围：指定 project_id 或 project_name 之一。如果提供了 project_name 但项目不存在，将自动创建。提供 project_id 时忽略此参数。如果两者都未提供，提示将在工作区级别创建。") @Nullable String projectName,
        ...
        @JsonView({Prompt.View.Public.class,
                Prompt.View.Write.class,
                Prompt.View.Detail.class}) @Schema(description = "模板结构类型：'text' 或 'chat'。创建后不可变。", defaultValue = "text") @Nullable TemplateStructure templateStructure,
        @JsonView({Prompt.View.Public.class,
                Prompt.View.Write.class,
                Prompt.View.Detail.class,
