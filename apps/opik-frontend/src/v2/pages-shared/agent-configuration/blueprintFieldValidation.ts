...
const nonEmptyString = z
  .string()
  .min(1, i18next.t("common:blueprint.mustNotBeEmpty"));
...
const FIELD_SCHEMAS: Partial<Record<BlueprintValueType, z.ZodType>> = {
  ...
  [BlueprintValueType.FLOAT]: nonEmptyString.pipe(
    z.coerce.number({
      message: i18next.t("common:blueprint.mustBeAValidNumber"),
    }),
  ),
...
export const validateBlueprintFieldValue = (
  ...
  return result.success
    ? ""
    : result.error.issues[0]?.message ?? i18next.t("common:blueprint.invalid");
};
