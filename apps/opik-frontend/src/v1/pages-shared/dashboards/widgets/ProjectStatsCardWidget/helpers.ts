...
const getMetricTitleMap = (source: TRACE_DATA_TYPE): Record<string, string> => {
  ...
  return {
    ...
    span_count: isSpans
      ? i18next.t("projectStats.totalSpanCount")
      : i18next.t("projectStats.averageSpanCount"),
    llm_span_count: i18next.t("projectStats.averageLlmSpanCount"),
    error_count: i18next.t("projectStats.totalErrorCount", {
      source: sourceLabel,
    }),
    "duration.p50": i18next.t("projectStats.p50Duration", {
      source: sourceLabel,
    }),
    "duration.p90": i18next.t("projectStats.p90Duration", {
      source: sourceLabel,
    }),
    "duration.p99": i18next.t("projectStats.p99Duration", {
      source: sourceLabel,
    }),
    total_estimated_cost: isSpans
      ? i18next.t("projectStats.avgCostPerSpan")
      : i18next.t("projectStats.avgCostPerTrace"),
    total_estimated_cost_sum: i18next.t("projectStats.totalCostSum", {
      source: sourceLabel,
    }),
    "usage.completion_tokens": i18next.t("projectStats.avgOutputTokens", {
      source: sourceLabel,
    }),
    "usage.prompt_tokens": i18next.t("projectStats.avgInputTokens", {
      source: sourceLabel,
    }),
    "usage.total_tokens": i18next.t("projectStats.avgTotalTokens", {
      source: sourceLabel,
    }),
    ...
    metadata: i18next.t("projectStats.totalMetadataCount", {
      source: sourceLabel,
    }),
    tags: i18next.t("projectStats.avgTagsCount", { source: sourceLabel }),
    guardrails_failed_count: i18next.t("projectStats.totalGuardrailsFailed", {
      source: sourceLabel,
    }),
  };
};
...
const calculateProjectStatsCardTitle = (
  ...
  if (metric.startsWith(FEEDBACK_SCORE_PREFIX)) {
    ...
    return i18next.t("projectStats.averageFeedbackScore", {
      source: sourceLabel,
      scoreName,
    });
  }
